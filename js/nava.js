...
$(function () {
    ...
    if (typeof navaObject !== 'undefined') {
        austDay = new Date(navaObject.year, parseInt(navaObject.month-1), navaObject.day, navaObject.hour, navaObject.min, navaObject.sec);
...
jQuery(document).ready(function($) {
    nonce = $('#review_nonce').val();
    postid = $('[name="review_id"]').val();
    $(document).on('click', "#submitReview", function(e) {

        e.preventDefault();
        $('.review-loading').removeClass('hide');
        $.ajax({
            url: AjaxObj.ajaxUrl,
            dataType: 'html',
            type: 'POST',
            data: {
                action: "save_review_rating",
                post_id: postid,
                review_opt1: $("[name='review_opt1']").val(),
                review_opt2: $("[name='review_opt2']").val(),
                review_opt3: $("[name='review_opt3']").val(),
                review_opt4: $("[name='review_opt4']").val(),
                review_opt5: $("[name='review_opt5']").val(),
                nonce: nonce,
            ...
            success: function (response) {
                ...
                for($i = 0; $i < $json.length ; $i++) {
                    cavg = parseFloat($json[$i].avg);
                    sum += cavg;
                    $('#ex'+($i+1)+'Slider' ).remove();
                    $(".rate-number" + ($i+1)).text(cavg.toFixed(1));
                }
                average = parseFloat(sum / $json.length);
                average = parseFloat(average.toFixed(1));
                avgDiv10 = average / 10;
                $("#reviewTotalResult").attr('data-value', avgDiv10.toFixed(1)).find('strong').text(average);

                $('.review-loading').addClass('hide');
            },
            error: function(xhr, stat, error) {
                console.log(stat + ' ' + error + ' ' + xhr );
            }
        });
    });
});
